...
export class SpotifyIntelligence {
  ...
  computeTasteDrift(data, historyEvents) {
    ...
    // Merge historical rising/fading artists into the top-level lists
    // History data is more reliable (real play counts), so it takes priority
    let mergedEmerging = emerging.slice(0, 10).map((a) => ({ name: a.name, genres: a.genres || [], source: 'api' }));
    let mergedFading = fading.slice(0, 10).map((a) => ({ name: a.name, genres: a.genres || [], source: 'api' }));

    if (historicalDrift) {
      const emergingNames = new Set(mergedEmerging.map((a) => a.name));
      for (const a of historicalDrift.risingArtists || []) {
        if (!emergingNames.has(a.name)) {
          mergedEmerging.push({ name: a.name, source: 'history', shareChange: `${a.share12m}%→${a.share3m}%`, momentum: a.momentum });
          emergingNames.add(a.name);
        }
      }
      const fadingNames = new Set(mergedFading.map((a) => a.name));
      for (const a of historicalDrift.fadingArtists || []) {
        if (!fadingNames.has(a.name)) {
          mergedFading.push({ name: a.name, source: 'history', shareChange: `${a.share12m}%→${a.share3m}%` });
          fadingNames.add(a.name);
        }
      }
    }
  ...
  computeHistoricalDrift(events) {
    ...
    console.log(`[Spotify Brainer] Historical drift: ${meaningful.length} meaningful events, 12m: ${last12m.length}, 3m: ${last3m.length}, 1m: ${last1m.length}`);
    if (last12m.length < 50 || last3m.length < 20) {
      console.warn(`[Spotify Brainer] Not enough recent history for drift (need 50 in 12m, 20 in 3m)`);
      return null;
    }
