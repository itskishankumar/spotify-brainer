...
/**
 * Get all cached artist tags as { artistName: [tag1, tag2, ...] }.
 * Used by realtime anchors to score genres without making API calls.
 */
export function getCachedArtistTags() {
  const result = {};
  for (const [key, entry] of Object.entries(tagCache)) {
    if (entry.tags?.length) {
      result[key] = entry.tags.map((t) => t.name);
    }
  }
  return result;
}
